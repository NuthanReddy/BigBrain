...
class NotionImporter:
    ...
    _BIGBRAIN_MARKER = "Generated by BigBrain"

    def import_page(self, page_id: str) -> Document | None:
        """Import a single Notion page into the KB.

        Skips pages that were exported by BigBrain (contain the marker callout).
        Returns the Document created/updated, or None on failure/skip.
        """
        try:
        ...
        # Skip pages exported by BigBrain
        if self._has_bigbrain_marker(blocks):
            title = self._client.get_page_title(page) or page_id[:8]
            logger.info("Skipping BigBrain-exported page: %s", title)
            return None
    ...
    @classmethod
    def _has_bigbrain_marker(cls, blocks: list[dict[str, Any]]) -> bool:
        """Check if the first few blocks contain the BigBrain export marker."""
        for block in blocks[:5]:
            btype = block.get("type", "")
            if btype == "callout":
                rich_text = block.get("callout", {}).get("rich_text", [])
                text = "".join(t.get("plain_text", "") for t in rich_text)
                if cls._BIGBRAIN_MARKER in text:
                    return True
        return False
