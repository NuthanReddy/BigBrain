...
x D x:right
return x
Both of these procedures run in O.h/ time on a tree of height h since, as in TREESEARCH, the sequence of nodes encountered forms a simple path downward from
the root.
...
node x as the node returned by calls made to TREE-SUCCESSOR.x/ and TREEPREDECESSOR.x/, respectively.
...
Figure 12.3 shows how TREE-INSERT works. Just like the procedures TREESEARCH and ITERATIVE-TREE-SEARCH, TREE-INSERT begins at the root of the
...
12.3-3
We can sort a given set of n numbers by ﬁrst building a binary search tree containing these numbers (using TREE-INSERT repeatedly to insert the numbers one by
...
between the building of a binary search tree and the execution of RANDOMIZEDQUICKSORT from Section 7.3.
