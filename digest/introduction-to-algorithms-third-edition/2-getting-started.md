...
We present our pseudocode for insertion sort as a procedure called INSERTIONSORT, which takes as a parameter an array AŒ1 : : n containing a sequence of
...
In the following discussion, our expression for the running time of INSERTIONSORT will evolve from a messy formula that uses all the statement costs ci to a
...
Lines 8–9 put the sentinels at the ends of the arrays L and R. Lines 10–17, illus32
A
L
R
i
...
We can now use the MERGE procedure as a subroutine in the merge sort algorithm. The procedure MERGE-SORT.A; p; r/ sorts the elements in the subarray AŒp : : r. If p  r, the subarray has at most one element and is therefore
...
MERGE-SORT.A; 1; A:length/, where once again A:length D n. Figure 2.4 illustrates the operation of the procedure bottom-up when n is a power of 2. The
