...
:
(A.5)
When the summation is inﬁnite and jxj < 1, we have the inﬁnite decreasing geometric series
X
kD0
xk D
1  x :
...
mathematical induction. Instead, you can use induction to prove a bound on a summation. As an example, let us prove that the geometric series Pn
kD0 3k is O.3n/.
...
We have to be careful when we use asymptotic notation to prove bounds by induction. Consider the following fallacious proof that Pn
...
kDk0 ak. This technique applies to inﬁnite summations as well. For example, to ﬁnd an asymptotic
upper bound on
X
kD0
k2
...
kDm f .k/, where f .k/ is a monotonically increasing function, we can approximate it by integrals:
Z n
m1
f .x/ dx 
n
...
within the rectangle, and the total rectangle area represents the value of the summation. The integral is represented by the shaded area under the curve.
