...
(see Exercise 2.1-3) would create just one subproblem containing only one element fewer than the original problem.
...
In practice, we neglect certain technical details when we state and solve recurrences. For example, if we call MERGE-SORT on n elements when n is odd, we
...
Similar to FIND-MAX-CROSSING-SUBARRAY, the recursive procedure FINDMAXIMUM-SUBARRAY returns a tuple containing the indices that demarcate a
...
Next we set up a recurrence that describes the running time of the recursive FINDMAXIMUM-SUBARRAY procedure. As we did when we analyzed merge sort in
...
result to be an empty subarray, where the sum of the values of an empty subar4.2
...
execution of line 7 takes constant time, the SQUARE-MATRIX-MULTIPLY procedure takes ‚.n3/ time.
...
O.n2:81/ time, which is asymptotically better than the simple SQUARE-MATRIXMULTIPLY procedure.
...
in place).
Now, we derive a recurrence to characterize the running time of SQUAREMATRIX-MULTIPLY-RECURSIVE. Let T .n/ be the time to multiply two n 	 n
...
Since the number of matrix additions is a constant, the total time spent adding ma78
...
assume that once the matrix size n gets down to 1, we perform a simple scalar multiplication, just as in line 4 of SQUARE-MATRIX-MULTIPLY-RECURSIVE. When
...
Strassen’s method is asymptotically faster than the straightforward SQUAREMATRIX-MULTIPLY procedure. The notes at the end of this chapter discuss some
...
For example, in the recurrence (4.19) we can falsely “prove” T .n/ D O.n/ by guessing T .n/  cn and
then arguing
T .n/

2.c bn=2c/ C n
...
4.3-3
We saw that the solution of T .n/ D 2T .bn=2c/Cn is O.n lg n/. Show that the solution of this recurrence is also .n lg n/. Conclude that the solution is ‚.n lg n/.
4.3-4
...
The ﬁrst part analyzes the master recurrence (4.20), under the simplifying assumption that T .n/ is deﬁned only on exact powers of b > 1, that is, for n D 1; b; b2; : : :. This part gives all the intuition
...
3. Strassen’s algorithm is not quite as numerically stable as SQUARE-MATRIXMULTIPLY. In other words, because of the limited precision of computer arithmetic on noninteger values, larger errors accumulate in Strassen’s algorithm
...
T .bn=3c/ C T .b2n=3c/ C O.n/, the Akra-Bazzi method does. To solve the recurrence (4.30), we ﬁrst ﬁnd the unique real number p such that Pk
iD1 aibp
i D 1.
