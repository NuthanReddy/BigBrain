...
appendix reviews more completely the notations, deﬁnitions, and elementary properties of sets, relations, functions, graphs, and trees. If you are already well versed
...
of R are nonempty, pairwise-disjoint sets whose union is A. Because R is reﬂexive, a 2 Œa, and so the equivalence classes are nonempty; moreover, since every
...
equivalence relation on A. Reﬂexivity holds, since a 2 Ai implies a R a. Symmetry holds, because if a R b, then a and b are in the same set Ai, and hence b R a.
...
total order.
B.2-2
Show that for any positive integer n, the relation “equivalent modulo n” is an equivalence relation on the integers. (We say that a  b .mod n/ if there exists an
...
differences are slight. Section 22.1 shows how we can represent graphs in computer memory.
...
Many deﬁnitions for directed and undirected graphs are the same, although certain terms have slightly different meanings in the two contexts. If .u; / is an edge
in a directed graph G D .V; E/, we say that .u; / is incident from or leaves
vertex u and is incident to or enters vertex . For example, the edges leaving vertex 2 in Figure B.2(a) are .2; 2/, .2; 4/, and .2; 5/. The edges entering vertex 2 are
...
number of edges entering it. The degree of a vertex in a directed graph is its in1170
Appendix B
Sets, Etc.
...
A subpath of path p D h0; 1; : : : ; ki is a contiguous subsequence of its vertices. That is, for any 0  i  j  k, the subsequence of vertices hi; iC1; : : : ; ji
is a subpath of p.
...
each other. The strongly connected components of a directed graph are the equiv4Some authors refer to what we call a path as a “walk” and to what we call a simple path as just a
...
The vertices f3; 6g do not form a strongly connected component, since vertex 6 cannot
...
In other words, we can relabel the vertices of G to be vertices of G0, maintaining the corresponding edges in G and G0. Figure B.3(a) shows a pair of isomorphic graphs G and G0 with respective vertex sets V D f1; 2; 3; 4; 5; 6g and
V 0 D fu; ; w; x; y; ´g. The mapping from V to V 0 given by f .1/ D u; f .2/ D ;
f .3/ D w; f .4/ D x; f .5/ D y; f .6/ D ´ provides the required bijective function. The graphs in Figure B.3(b) are not isomorphic. Although both graphs have
...
There are two variants of graphs that you may occasionally encounter. A multigraph is like an undirected graph, but it can have both multiple edges between vertices and self-loops. A hypergraph is like an undirected graph, but each hyperedge,
...
If k < jV j, there must be a vertex kC1 2 V  Vk that is adjacent to some vertex i 2 Vk, since G is connected. Deﬁne GkC1 D .VkC1; EkC1/ to be the subgraph of G with VkC1 D Vk [ fkC1g and EkC1 D Ek [ f.i; kC1/g. Note that
...
by deﬁnition, and since (1) implies (5), the sum of all edges in all connected components of G is jV j  k. Consequently, we must have k D 1, and G is in fact a
...
of the child—whether it is the left child or the right child—matters. In an ordered tree, there is no distinguishing a sole child as being either left or right. Figure B.7(b) shows a binary tree that differs from the tree in Figure B.7(a) because of
...
node is either a leaf or has degree exactly 2. There are no degree-1 nodes. Consequently, the order of the children of a node preserves the position information.
