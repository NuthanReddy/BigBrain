...
extract p bits
Figure 11.4
The multiplication method of hashing. The w-bit representation of the key k is multiplied by the w-bit value s D A  2w. The p highest-order bits of the lower w-bit half of the product
...
˛ D O.1/. The INSERT and DELETE operations take constant time and, by Theorem 11.3, the expected time for each SEARCH operation is O.1/. By linearity of
...
and let Z
p denote the set f1; 2; : : : ; p  1g. Since p is prime, we can solve equations modulo p with the methods given in Chapter 31. Because we assume that the
...
j to provide collision-free constanttime lookup.
...
that p.n; m/  en.n1/=2m. (Hint: See equation (3.12).) Argue that when n exceeds pm, the probability of avoiding collisions goes rapidly to zero.
...
hx0; x1; : : : ; xn1i 2 U . For any n-tuple a D ha0; a1; : : : ; an1i 2 U , deﬁne the hash function ha by
ha.x/ D
n1
X
jD0
...
Argue that the probability that the adversary succeeds in fooling Bob into accepting .m0; t0/ is at most 1=p, no matter how much computing power the adversary has, and even if the adversary knows the family H of hash functions
used.
